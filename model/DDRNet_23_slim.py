...
class DualResNet(nn.Module):
    def __init__(self, block, layers, out_features=19, planes=64, spp_planes=128,
                 head_planes=128, augment=False, skip_mode='none'):
        super(DualResNet, self).__init__()

        highres_planes = planes * 2
        self.augment = augment
        # skip_mode controls which intermediate encoder feature (if any) is
        # additionally returned by forward(), for use as a semantic
        # skip-connection in the decoder. Options: 'none', 'single', 'wide', 'deep'.
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        layers.append(x)                     # layers[0]: (B, planes,    H/4,  W/4)

        x = self.layer2(self.relu(x))
        layers.append(x)                     # layers[1]: (B, planes*2,  H/8,  W/8)

        x = self.layer3(self.relu(x))
        layers.append(x)                     # layers[2]: (B, planes*4,  H/16, W/16)
        ...
        if self.skip_mode == 'none':
            return x_
        elif self.skip_mode == 'single':
            return x_, layers[0]
        elif self.skip_mode == 'wide':
            return x_, layers[1]
        elif self.skip_mode == 'deep':
            return x_, layers[2]
        elif self.skip_mode == 'multi':
            return x_, layers[0], layers[1], layers[2]
        else:
            raise ValueError(f"Unknown skip_mode: '{self.skip_mode}'. "
                              f"Expected one of: 'none', 'single', 'wide', 'deep', 'multi'.")


def DualResNet_Backbone(pretrained=False, features=64, skip_mode='none'):
