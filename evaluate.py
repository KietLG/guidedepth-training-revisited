...
class Evaluater():
    def __init__(self, args):
        ...
        self.spatial_attention = getattr(args, 'spatial_attention', False)
        self.skip_connection = getattr(args, 'skip_connection', 'single')
        ...
        self.confirm_test_eval = getattr(args, 'confirm_test_eval', False)
    ...
    def evaluate(self):
        if not self.confirm_test_eval:
            print("=" * 80)
            print("ERROR: Test set evaluation is BLOCKED by default to prevent overfit.")
            print("To confirm and run the evaluation, set 'confirm_test_eval: true' in config.yaml")
            print("or run with --confirm_test_eval flag.")
            print("=" * 80)
            raise RuntimeError("Test set evaluation blocked by safeguard.")
        ...
        for i, data in enumerate(self.test_loader):
            ...
            # Single forward pass — KHÔNG multi-scale TTA, khớp đúng protocol của paper
            # (xem evaluate.py gốc: chỉ self.model(image) và self.model(image_flip)).
            with torch.no_grad():
    ...
    def save_results(self, average):
        results_file = os.path.join(self.result_dir, 'evaluation_result.txt')
        with open(results_file, 'w') as f:
            f.write(f'skip_connection: {self.skip_connection}\n')
            f.write(f'spatial_attention: {self.spatial_attention}\n')
